...
public abstract class Hero {
    ...
    //能否普通攻击
    private boolean canReduceHp = true;
    ...
    public boolean iscanReduceHp() {
        return canReduceHp;
    }

    public void setcanReduceHp(boolean canReduceHp) {
        this.canReduceHp = canReduceHp;
    }
    ...
    public final void cast(Skill skill, Hero hero) {
        ...
        if (this.mp >= skill.getConsume()) {
            ...
            if (this.isCanDamage()) {
                ...
                System.out.println("【"+this.name+"】没蓝了");
            }
        }
    }
    ...
    public void reduceHp(int num) {
        if(this.iscanReduceHp())
        {this.hp -= num;}
        else {
            System.out.println(this.name+"处于无敌状态");
        }
